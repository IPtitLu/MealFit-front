<template>
  <div class="w-full flex justify-center items-center">
    <div class="w-4/5 grid grid-cols-2 gap-8">
      <div class="w-full">
        <!-- Input -->
        <div class="w-full flex justify-center gap-2 items-center flex-col mb-10 px-6 rounded-md py-4 bg-white drop-shadow-lg pt-6 pb-10">
          ...
          <div class="flex flex-wrap gap-4">
            <div v-for="(ingredient, index) in ingredients" :key="index"
              class="relative flex items-center space-x-3 rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm focus-within:ring-2 focus-within:ring-primary-700 focus-within:ring-offset-2">
              <div>
          ...
          <div @click="validateIngredients" v-if="isValidateButtonVisible"
              class="relative flex items-center space-x-3 rounded-lg bg-secondary-100 hover:bg-secondary-200 border border-gray-300 bg-white px-6 py-5 shadow-sm focus-within:ring-2 focus-within:ring-primary-700 focus-within:ring-offset-2 hover:border-primary-700 cursor-pointer">
          ...
          <button @click="transferIngredients" v-if="ingredients.length > 0" class="bg-secondary-100 hover:bg-secondary-200 border border-gray-300 text-primary-700 py-1.5 px-4 rounded">
            Valider
          </button>
        </div>
      </div>
          ...
          <div class="mt-8 flow-root">
            <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
              <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                <table class="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(ingredient, index) in validatedIngredients" :key="index"
              ...
              <div class="text-center">
                <button @click="validateIngredients" v-if="validatedIngredients.length > 0" class="bg-secondary-100 hover:bg-secondary-200 border border-gray-300 text-primary-700 py-1.5 px-4 rounded">
                  Valider
                </button>
              </div>
            </div>
...
const validateIngredients = async () => {
  const ingredientsArray = Object.values(validatedIngredients.value);
  await addGroceryItem(userId, ingredientsArray);

  validatedIngredients.value = [];

  await getGroceryList(userId);
};

const removeValidateIngredient = async (index) => {
  await updateGroceryItem(userId, validatedIngredients.value[index].id, {
    quantity: 0,
    unit: '',
  });
